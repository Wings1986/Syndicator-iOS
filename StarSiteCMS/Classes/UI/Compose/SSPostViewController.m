...
@interface SSPostViewController ()

@end

@implementation SSPostViewController
...
- (IBAction)doShare{
    ...
    if(!_capturedImage && !_isVideo){
        ...
        [[SSAppController sharedInstance] doShareWithDictionary:dict];
        [[SSAppController sharedInstance] hideKeyboard];
...
- (IBAction)playVideo {
    ...
    if(!_isVideo)
        return;
    NSLog(@"playVideo1 : %@",_videoURL.absoluteString);
